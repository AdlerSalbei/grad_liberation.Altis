switch (liberation_preset_blufor) do {
...
switch (liberation_preset_opfor) do {
...
switch (liberation_preset_resistance) do {
...
switch (liberation_preset_civilians) do {
...
infantry_units 																	= [infantry_units] call grad_liberation_common_fnc_filterMods;
light_vehicles 																	= [light_vehicles] call grad_liberation_common_fnc_filterMods;
heavy_vehicles 																	= [heavy_vehicles] call grad_liberation_common_fnc_filterMods;
air_vehicles 																	= [air_vehicles] call grad_liberation_common_fnc_filterMods;
support_vehicles 																= [support_vehicles] call grad_liberation_common_fnc_filterMods;
static_vehicles 																= [static_vehicles] call grad_liberation_common_fnc_filterMods;
buildings 																		= [buildings] call grad_liberation_common_fnc_filterMods;
build_lists 																	= [[],infantry_units,light_vehicles,heavy_vehicles,air_vehicles,static_vehicles,buildings,support_vehicles,squads];
liberation_storage_buildings 													= [liberation_small_storage_building,liberation_large_storage_building];
liberation_crates 																= [liberation_supply_crate,liberation_ammo_crate,liberation_fuel_crate];
liberation_upgrade_buildings 													= [liberation_recycle_building,liberation_air_vehicle_building,liberation_heli_slot_building,liberation_plane_slot_building];
liberation_air_slots 															= [liberation_heli_slot_building,liberation_plane_slot_building];
militia_squad 																	= [militia_squad, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
militia_vehicles 																= [militia_vehicles, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_vehicles 																	= [opfor_vehicles, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_vehicles_low_intensity 													= [opfor_vehicles_low_intensity, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_battlegroup_vehicles 														= [opfor_battlegroup_vehicles, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_battlegroup_vehicles_low_intensity 										= [opfor_battlegroup_vehicles_low_intensity, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_troupTransports 															= [opfor_troupTransports, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_choppers 																	= [opfor_choppers, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
opfor_air 																		= [opfor_air, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
civilians 																		= [civilians, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
civilian_vehicles 																= [civilian_vehicles, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
...
{
	if (!(_x in friendly_infantry_classnames)) then {
		friendly_infantry_classnames pushBack _x
	};
} forEach (blufor_squad_inf_light + blufor_squad_inf + blufor_squad_at + blufor_squad_aa + blufor_squad_recon + blufor_squad_para);

{
	if (!((_x select 0) in friendly_infantry_classnames)) then {
		friendly_infantry_classnames pushBack (_x select 0)
	};
} forEach infantry_units;

all_hostile_classnames 															= (land_vehicles_classnames + opfor_air + opfor_choppers + opfor_troupTransports + opfor_vehicles_low_intensity);
{
	land_vehicles_classnames pushback (_x select 0);
} foreach (heavy_vehicles + light_vehicles);
air_vehicles_classnames 														= [] + opfor_choppers;
liberation_friendly_air_classnames 												= [];
{
	air_vehicles_classnames pushback (_x select 0);
	liberation_friendly_air_classnames pushback (_x select 0);
} foreach air_vehicles;
liberation_friendly_air_classnames 												= liberation_friendly_air_classnames select {!(_x call grad_liberation_common_fnc_isClassUAV)};
liberation_static_classnames 													= [];
{
	liberation_static_classnames pushback (_x select 0);
...
ammobox_transports_typenames 													= [ammobox_transports_typenames, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
elite_vehicles 																	= [elite_vehicles, {[_x] call grad_liberation_common_fnc_checkClass}] call BIS_fnc_conditionalSelect;
