...
if (combat_readiness > 15) then {
	...
	if (_targetsector in active_sectors) then {
		...
		if (_nearestower != "") then {
			...
			if ((_targetsector in active_sectors) && !(_targetsector in blufor_sectors) && !(_nearestower in blufor_sectors) && (!([] call grad_liberation_shared_fnc_isBigtownActive) || _targetsector in sectors_bigtown)) then {
