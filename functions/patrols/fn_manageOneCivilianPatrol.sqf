...
while { LIB_endgame == 0 } do {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		_grp = createGroup LIB_side_civilian;
		...
		{ _x addEventHandler ["HandleDamage", { private [ "_damage" ]; if (( side (_this select 3) != LIB_side_friendly ) && ( side (_this select 3) != LIB_side_enemy )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ]; } foreach units _grp;
