...
while { GRLIB_endgame == 0 } do {
	...
	_usable_sectors = [];
	{
		if ( ( ( [ getmarkerpos _x , 1000 , GRLIB_side_friendly ] call grad_liberation_shared_fnc_getUnitsCount ) == 0 ) && ( count ( [ getmarkerpos _x , 3500 ] call grad_liberation_shared_fnc_getNearbyPlayers ) > 0 ) ) then {
