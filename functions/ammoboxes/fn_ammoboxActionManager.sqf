...
while {true} do {
	...
	if ([player, 5] call grad_liberation_common_fnc_fetchPermission) then {
		...
		_nearammoboxes = ((getpos player) nearEntities [liberation_crates,10]);
		_neartransporttrucks = ((getpos player) nearEntities [ammobox_transports_typenames,10]);
		_nearstorageareas = nearestObjects [player,liberation_storage_buildings,10];
...

		{
			...
			if (!(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
					...
					_next_truck setVariable [ "LIB_ammo_truck_action", _action_id, false ];
					_managed_trucks pushback _next_truck;
			};

			if ((_next_truck in _managed_trucks) && _truck_load == 0) then {
				_next_truck removeAction (_next_truck getVariable ["LIB_ammo_truck_action", -1]);
...

		{
			...
			if (!(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
				...
				_b_action_id2 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_STORE_CRATE" + "</t>",{[(_this select 0), (nearestObjects [player,liberation_storage_buildings,20]) select 0,true] call grad_liberation_common_fnc_crateToStorage;},"",-502,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
				_b_action_id3 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_CRATE_VALUE" + "</t>",{[_this select 0] call grad_liberation_common_fnc_crateCheckValue;uiSleep 3; hint "";},"",-503,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
				...
				_next_box setVariable ["LIB_ammo_box_action", _b_action_id1, false];
...

		{
			...
			if (!(_next_box in _managed_boxes) || !( isNull  attachedTo _next_box )) then {
				...
				_next_box removeAction (_next_box getVariable ["LIB_ammo_box_action", -1]);
...

		{
			...
			if (!(_next_area in _managed_areas) && (_area_load > 0)) then {
					_action_id = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_SUPPLY" + "</t>",{[liberation_supply_crate, (_this select 0), true] spawn grad_liberation_common_fnc_crateToStorage;},"",-504,true,true,"","build_confirmed == 0 && (_this distance _target < 12) && (vehicle player == player)"];
					_next_area setVariable ["KP_supply_unstore_action", _action_id, false];
					_action_id2 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_AMMO" + "</t>",{[liberation_ammo_crate, (_this select 0), true] spawn grad_liberation_common_fnc_crateToStorage;},"",-505,true,true,"","build_confirmed == 0 && (_this distance _target < 12) && (vehicle player == player)"];
					_next_area setVariable ["KP_ammo_unstore_action", _action_id2, false];
					_action_id3 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_UNSTORE_FUEL" + "</t>",{[liberation_fuel_crate, (_this select 0), true] spawn grad_liberation_common_fnc_crateToStorage;},"",-506,true,true,"","build_confirmed == 0 && (_this distance _target < 12) && (vehicle player == player)"];
					_next_area setVariable ["KP_fuel_unstore_action", _action_id3, false];
					_action_id4 = _next_area addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_SORT_STORAGE" + "</t>",{[(_this select 0)] spawn grad_liberation_common_fnc_sortStorage;},"",-507,true,true,"","build_confirmed == 0 && (_this distance _target < 12) && (vehicle player == player)"];
