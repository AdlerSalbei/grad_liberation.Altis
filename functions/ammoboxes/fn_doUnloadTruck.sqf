...
if ( _truck_to_unload getVariable ["LIB_ammo_truck_load", 0] > 0 ) then {
	...
	[_truck_to_unload, false] remoteExec ["grad_liberation_common_fnc_objectProtection"];

	{
		_next_box = _x;
		[_next_box, false] remoteExec ["grad_liberation_common_fnc_objectProtection"];
		sleep 0.5;
		detach _next_box;
		...
		_offset = _offset - 2.2;
		sleep 0.5;
		[_next_box, true] remoteExec ["grad_liberation_common_fnc_objectProtection"];
		[_next_box, true] remoteExec ["enableRopeAttach"];
	} foreach ( attachedObjects _truck_to_unload);

	sleep 0.5;

	[_truck_to_unload, true] remoteExec ["grad_liberation_common_fnc_objectProtection"];
