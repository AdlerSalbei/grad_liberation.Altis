...
while { dialog && alive player } do {
	...
	if (  { alive _x } count (units group player) != _membercount ) then {
		...
		{
			if ( alive _x ) then {
				_unitname =  format ["%1. ", [ _x ] call grad_liberation_common_fnc_getUnitPositionId];
				if(isPlayer _x) then {
	...
	if ( !(isNull _selectedmember) ) then {
		if ( _memberselection != lbCurSel 101 || _resupplied || ( ( vehicle _selectedmember == _selectedmember && _isvehicle ) || ( vehicle _selectedmember != _selectedmember && !_isvehicle ) ) ) then {
			...
			_unitname = format ["%1. ", [ _selectedmember ] call grad_liberation_common_fnc_getUnitPositionId];
	...
	if( LIB_squadconfirm == 0 ) then {
		LIB_squadconfirm = -1;
		LIB_squadaction = -1;
	};

	if ( LIB_squadconfirm == 1 ) then {
		LIB_squadconfirm = -1;

		if ( LIB_squadaction == 1 ) then {

			_nearfob = [ getpos _selectedmember ] call grad_liberation_common_fnc_getNearestFob;
			_fobdistance = 9999;
			...
			if ( _fobdistance < 100 || count _nearsquad > 0 ) then {
				...
				_tempgmp = createGroup LIB_side_friendly;
				(typeof _selectedmember) createUnit [ markers_reset, _tempgmp,''];
				[ (units _tempgmp) select 0, _selectedmember ] call grad_liberation_common_fnc_swapInventory;
		...
		if (LIB_squadaction == 2) then {
			deleteVehicle _selectedmember;
			_resupplied = true;
			hint localize 'STR_REMOVE_OK';
		};

		if (LIB_squadaction == 3) then {

			closeDialog 0;

			if ( primaryWeapon player == "" && secondaryWeapon player == "" ) then {
				[ _selectedmember, player ] call grad_liberation_common_fnc_swapInventory;
			};
