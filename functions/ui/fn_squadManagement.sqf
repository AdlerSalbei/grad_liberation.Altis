...
while { dialog && alive player } do {
	...
	if (  { alive _x } count (units group player) != _membercount ) then {
		...
		{
			if ( alive _x ) then {
				_unitname =  format ["%1. ", [ _x ] call grad_liberation_shared_fnc_getUnitPositionId];
				if(isPlayer _x) then {
	...
	if ( !(isNull _selectedmember) ) then {
		if ( _memberselection != lbCurSel 101 || _resupplied || ( ( vehicle _selectedmember == _selectedmember && _isvehicle ) || ( vehicle _selectedmember != _selectedmember && !_isvehicle ) ) ) then {
			...
			_unitname = format ["%1. ", [ _selectedmember ] call grad_liberation_shared_fnc_getUnitPositionId];
	...
	if ( LIB_squadconfirm == 1 ) then {
		...
		if ( LIB_squadaction == 1 ) then {
			...
			_nearfob = [ getpos _selectedmember ] call grad_liberation_shared_fnc_getNearestFob;
			_fobdistance = 9999;
			...
			if ( _fobdistance < 100 || count _nearsquad > 0 ) then {
				...
				[ (units _tempgmp) select 0, _selectedmember ] call grad_liberation_shared_fnc_swapInventory;
		...
		if (LIB_squadaction == 3) then {
			...
			if ( primaryWeapon player == "" && secondaryWeapon player == "" ) then {
				[ _selectedmember, player ] call grad_liberation_shared_fnc_swapInventory;
			};
