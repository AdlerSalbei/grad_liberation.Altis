...
if ( isNil "LIB_secondary_in_progress" ) then { LIB_secondary_in_progress = -1; };
...
while { dialog && alive player && dostartsecondary == 0 } do {
	...
	_missioncost = LIB_secondary_missions_costs select _oldchoice;

	if ( ( _missioncost <= resources_intel ) && ( !LIB_secondary_starting ) )  then {
		ctrlEnable [ 103, true ];
