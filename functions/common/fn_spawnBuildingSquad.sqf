...
if (_infsquad == "militia") then {
	...
	_infsquad_classnames = (["army"] call grad_liberation_common_fnc_getAdaptiveSquadComp);
};
...
private _idxposit = 0;
{
	...
	_nextunit addMPEventHandler ["MPKilled", {_this spawn [] call grad_liberation_common_fnc_killManager}];
	_nextunit setdir (random 360);
	_nextunit setpos (_buildingpositions select (_position_indexes select _idxposit));
	[_nextunit, _sector] spawn buildingDefenceAi;
