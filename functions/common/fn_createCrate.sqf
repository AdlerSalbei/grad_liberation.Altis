/*
grad_liberation_common_fnc_createCrate.sqf
Author: Wyqer
...
params [["_resource", liberation_supply_crate],["_amount", 100],["_pos", getPos player]];

if !(_resource in liberation_crates) exitWith {
	private _text = format ["[KP LIBERATION] [ERROR] grad_liberation_common_fnc_createCrate.sqf -> invalid _resource: %1", _resource];
	_text remoteExec ["diag_log",2];
};

private _crate = _resource createVehicle _pos;
_crate setMass 500;
_crate setVariable ["liberation_crate_value", _amount, true];
