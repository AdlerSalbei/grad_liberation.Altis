...
private _corrected_amount = round ((count _squadies_to_spawn) * ([] call grad_liberation_common_fnc_adaptiveOpforFactor));
private _grp = createGroup LIB_side_enemy;

{
	if ((count (units _grp)) < _corrected_amount) then {
		_x createUnit [_spawnpos, _grp, "this addMPEventHandler [""MPKilled"", {_this spawn [] call grad_liberation_common_fnc_killManager}]"];
	};
	sleep 0.1;
