...
if (isServer) then {
	...
	if ((side _killer) == LIB_side_friendly) then {
		...
		if (count _nearby_bigtown > 0) then {
			combat_readiness = combat_readiness + (0.5 * liberation_difficulty_modifier);
			stats_readiness_earned = stats_readiness_earned + (0.5 * liberation_difficulty_modifier);
			if (combat_readiness > 100.0 && liberation_difficulty_modifier < 2) then {combat_readiness = 100.0};
		};
	...
	if (_unit isKindOf "Man") then {
		if (side (group _unit) == LIB_side_civilian) then {
			...
			if (side _killer == LIB_side_friendly) then {
				...
				[2, [(name _unit)]] remoteExec ["grad_liberation_common_fnc_globalMsg"];
				[liberation_cr_kill_penalty, true] spawn F_cr_changeCR;
			};
		...
		if (side (group _unit) == LIB_side_resistance) then {
			...
			if (((LIB_side_friendly getFriend LIB_side_resistance) >= 0.6) && (side _killer == LIB_side_friendly)) then {
				...
				[3, [(name _unit)]] remoteExec ["grad_liberation_common_fnc_globalMsg"];
				[liberation_cr_resistance_penalty, true] spawn F_cr_changeCR;
			};
		};
	} else {
...
if (isServer && !isplayer _unit) then {
	if (((typeof _unit) == liberation_ammo_crate) && ((getPosATL _unit) select 2 < 10)) then {
