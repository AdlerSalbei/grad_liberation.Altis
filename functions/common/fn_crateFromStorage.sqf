/*
grad_liberation_common_fnc_crateFromStorage.sqf
Author: Wyqer
...
switch (typeOf _storage) do {
	case liberation_small_storage_building: {_storage_positions = liberation_small_storage_positions; _unload_distance = 4;};
	case liberation_large_storage_building: {_storage_positions = liberation_large_storage_positions; _unload_distance = 6.5;};
	default {_storage_positions = liberation_large_storage_positions; _unload_distance = 6.5;};
};

_i = 0;
_unload_pos = [_storage, _unload_distance, (getDir _storage) - 180] call BIS_fnc_relPos;

while {!((nearestObjects [_unload_pos,liberation_crates,1]) isEqualTo [])} do {
	_i = _i + 1;
...

{
	if (typeOf _x == _cratetype) then {
		...
		if(liberation_ace) then {[_x, true, [0, 1.5, 0], 0] remoteExec ["ace_dragging_fnc_setCarryable"];};
		_unloaded = true;
	};
