...
while {true} do {
	...
	_kp_markers_array = [];
	{
		if ((side _x == GRLIB_side_enemy) && (({!captive _x} count (units _x) ) > 0) && ([(getpos leader _x), GRLIB_side_friendly, GRLIB_radiotower_size] call grad_liberation_shared_fnc_getNearestTower != "")) then {
