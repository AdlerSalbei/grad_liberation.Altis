...
while {true} do {
	...
	_kp_markers_array = [];
	{
		if ((side _x == LIB_side_enemy) && (({!captive _x} count (units _x) ) > 0) && ([(getpos leader _x), LIB_side_friendly, LIB_radiotower_size] call grad_liberation_shared_fnc_getNearestTower != "")) then {
