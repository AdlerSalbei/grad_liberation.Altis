...
"opfor_bg_marker" setMarkerColorLocal LIB_color_enemy_bright;

createMarkerLocal ["opfor_capture_marker", markers_reset];
"opfor_capture_marker" setMarkerTypeLocal "mil_objective";
"opfor_capture_marker" setMarkerColorLocal LIB_color_enemy_bright;
...
while { true } do {
	...
	if ( sector_timer > 0 ) then {
		"opfor_capture_marker" setMarkerTextLocal format ["%1",([sector_timer] call grad_liberation_common_fnc_secondsToTimer)];
