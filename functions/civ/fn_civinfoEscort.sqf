...
if (liberation_civinfo_debug > 0) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_escort called on: %1 - Parameters: [%2]", debug_source, _informant];_text remoteExec ["diag_log",2];};

waitUntil {sleep 0.5; local _informant || !alive _informant};

if !(alive _informant) exitWith {if (liberation_civinfo_debug > 0) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_escort exited by: %1 - Informant isn't alive", debug_source];_text remoteExec ["diag_log",2];};};
...
if (alive _informant) then {
	if (_is_near_fob) then {
		...
		if (liberation_civinfo_debug > 0) then {private _text = "[KP LIBERATION] [CIVINFO] civinfo_escort -> Informant at FOB";_text remoteExec ["diag_log",2];};
		sleep 600;
		deleteVehicle _informant;
		if (liberation_civinfo_debug > 0) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_escort finished by: %1", debug_source];_text remoteExec ["diag_log",2];};
	};
};
