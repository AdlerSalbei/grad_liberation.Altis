...
if (liberation_civinfo_debug > 0) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_task.sqf -> vehicle and group created on: %1", debug_source];_text remoteExec ["diag_log",2];};
...
while {(alive _hvt) && _time_remaining > 0} do {
	...
	if ((liberation_civinfo_debug > 0) && ((_time_remaining % 60) == 0)) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_task.sqf -> Task will despawn in %1 minutes", round (_time_remaining / 60)];_text remoteExec ["diag_log",2];};
};

if (liberation_civinfo_debug > 0) then {private _text = format ["[KP LIBERATION] [CIVINFO] civinfo_task.sqf -> loop exited on: %1", debug_source];_text remoteExec ["diag_log",2];};

if (alive _hvt) then {
	deleteVehicle _veh;
	{deleteVehicle _x} forEach (units _grp);
	[6] remoteExec ["civinfo_notifications"];
	if (liberation_civinfo_debug > 0) then {private _text = "[KP LIBERATION] [CIVINFO] civinfo_task.sqf -> Task despawned";_text remoteExec ["diag_log",2];};
} else {
	...
	if (liberation_civinfo_debug > 0) then {private _text = "[KP LIBERATION] [CIVINFO] civinfo_task.sqf -> Task ended with success";_text remoteExec ["diag_log",2];};
};
