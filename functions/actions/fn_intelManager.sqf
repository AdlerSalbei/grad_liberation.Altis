...
while { true } do {
	...
	if (  [ player, 5 ] call grad_liberation_common_fnc_fetchPermission  ) then {
		...
		{
			if ((captive _x) && !(_x in _actionned_captive_units) && !((side group _x) == LIB_side_friendly) && !(_x getVariable ["ACE_isUnconscious", false])) then {
				_x addAction ["<t color='#FFFF00'>" + localize "STR_SECONDARY_CAPTURE" + "</t>","scripts\client\actions\do_capture.sqf","",-850,true,true,"","(vehicle player == player) && (side group _target != LIB_side_friendly) && (captive _target)"];
				_actionned_captive_units pushback _x;
			};
		} foreach _near_people;

		{
			if ( !(alive _x) || ((player distance _x) > 5) || ((side group _x) == LIB_side_friendly) ) then {
				removeAllActions _x;
