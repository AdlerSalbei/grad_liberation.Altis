...
liberation_resources_global = false;
...
while {true} do {
	...
	private _nearfob = [] call grad_liberation_common_fnc_getNearestFob;
	private _fobdistance = 9999;
	private _nearest_sector = [LIB_fob_range] call grad_liberation_common_fnc_getNearestSector;
	...
	if (liberation_mobilerespawn) then {liberation_nearspawn = ( (getpos player) nearEntities [ [ Respawn_truck_typename, huron_typename ] , _distspawn ] );} else {liberation_nearspawn = [];};
	...
	if ((_fobdistance < _distredeploy || (player distance startbase) < 200) && alive player && vehicle player == player && liberation_halo_param > 0) then {
	...
	if ((_fobdistance < _distredeploy || count liberation_nearspawn != 0 || (player distance startbase) < 200) && alive player && vehicle player == player) then {
	...
	if ((_fobdistance < _distredeploy || count liberation_neararsenal != 0 || count liberation_nearspawn != 0 || (player distance startbase) < 200) && alive player && vehicle player == player) then {
	...
	if (_fobdistance < _distfob && alive player && vehicle player == player && (([ player, 3] call grad_liberation_common_fnc_fetchPermission) || (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin))) then {
	...
	if ((count _nearfobbox != 0) && (alive player) && (vehicle player == player) && !(surfaceIsWater getpos player) && ((player distance startbase) > 1000) && (([player, 3] call grad_liberation_common_fnc_fetchPermission) || (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin)) && !FOB_build_in_progress) then {
	...
	if (_fobdistance < _distredeploy && alive player && vehicle player == player && (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin)) then {
	...
	if ((count LIB_all_fobs > 0) && (LIB_endgame == 0) && (_fobdistance < _distredeploy || (player distance startbase) < 200) && alive player && vehicle player == player && (([player, 5] call grad_liberation_common_fnc_fetchPermission) || (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin))) then {
	...
	if ((count _prod_sector) == 12) then {
		if (alive player && vehicle player == player && ([player, 3] call grad_liberation_common_fnc_fetchPermission) && ((count (_prod_sector select 3)) == 0)) then {
			if (_idact_sectorstorage == -1) then {
				_idact_sectorstorage = player addAction ["<t color='#FFFF00'>" + localize "STR_SECSTORAGEBUILD_ACTION" + "</t>","scripts\client\build\do_sector_build.sqf",[liberation_small_storage_building, _prod_sector],-993,false,true,"","build_confirmed == 0"];
			};
		} else {
			if (_idact_sectorstorage != -1) then {
		...
		if (alive player && vehicle player == player && ([player, 3] call grad_liberation_common_fnc_fetchPermission) && ((count (_prod_sector select 3)) == 3) && !(_prod_sector select 4)) then {
		...
		if (alive player && vehicle player == player && ([player, 3] call grad_liberation_common_fnc_fetchPermission) && ((count (_prod_sector select 3)) == 3) && !(_prod_sector select 5)) then {
		...
		if (alive player && vehicle player == player && ([player, 3] call grad_liberation_common_fnc_fetchPermission) && ((count (_prod_sector select 3)) == 3) && !(_prod_sector select 6)) then {
	...
	if (_fobdistance < _distfob && alive player && vehicle player == player) then {
		if (_idact_resources == -1) then {
			_idact_resources = player addAction ["<t color='#FFFF00'>" + localize "STR_RESOURCE_GLOBAL_ACTION" + "</t>",{liberation_resources_global = !liberation_resources_global},"",-997,false,true,"","build_confirmed == 0"];
		};
	} else {
		if (_idact_resources != -1) then {
	...
	if (((_fobdistance < _distfob) || ((count _prod_sector) == 12)) && (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin) && alive player && vehicle player == player && ((count liberation_production) > 0)) then {
	...
	if (liberation_ailogistics) then {
		if ((_fobdistance < _distfob) && (player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin) && alive player && vehicle player == player && (((count LIB_all_fobs) + (count liberation_production)) > 1)) then {
	...
	if ((player == ([] call grad_liberation_common_fnc_getCommander) || [] call grad_liberation_common_fnc_isAdmin) && alive player && vehicle player == player && liberation_permissions_param) then {
