if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] Module initialising on: %1", debug_source];_text remoteExec ["diag_log",2];};
...
// Start module loop
[] call grad_liberation_asymmetric_fnc_asymmetric_loop.sqf";

if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] Module initialised on: %1", debug_source];_text remoteExec ["diag_log",2];};
