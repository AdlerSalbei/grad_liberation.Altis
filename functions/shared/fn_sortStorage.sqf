/*
grad_liberation_shared_fnc_sortStorage.sqf
Author: Wyqer
...
while {_supply > 0} do {
	...
	private _crate = [KP_liberation_supply_crate, _amount, _pos] call grad_liberation_shared_fnc_createCrate;
	sleep 0.1;
...
while {_ammo > 0} do {
	...
	_ammo = _ammo - _amount;
	private _crate = [KP_liberation_ammo_crate, _amount, _pos] call grad_liberation_shared_fnc_createCrate;
	sleep 0.1;
	[_crate, _storage] call grad_liberation_shared_fnc_crateToStorage;
};
...
while {_fuel > 0} do {
	...
	_fuel = _fuel - _amount;
	private _crate = [KP_liberation_fuel_crate, _amount, _pos] call grad_liberation_shared_fnc_createCrate;
	sleep 0.1;
	[_crate, _storage] call grad_liberation_shared_fnc_crateToStorage;
};
