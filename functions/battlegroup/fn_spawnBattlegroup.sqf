...
if ( count _this == 1 ) then {
	_spawn_marker = [ 2000, 10000, false, _this select 0 ] call grad_liberation_common_fnc_findOpforSpawnPoint;
} else {
	_spawn_marker = [ 2000, 10000, false ] call grad_liberation_common_fnc_findOpforSpawnPoint;
};
...
if ( _spawn_marker != "" ) then {
	...
	_target_size = LIB_battlegroup_size * ([] call grad_liberation_common_fnc_adaptiveOpforFactor) * (sqrt liberation_csat_aggressivity);
...

	{
		...
		_vehicle = [markerpos _spawn_marker, _x] call grad_liberation_common_fnc_libSpawnVehicle;
		sleep 0.5;
		(crew _vehicle) joinSilent _nextgrp;
		[_nextgrp] spawn battlegroupAi;
		_bg_groups pushback _nextgrp;
		if ( ( _x in opfor_troupTransports ) &&  ( [] call grad_liberation_common_fnc_opforCap < LIB_battlegroup_cap ) ) then {
			[_vehicle] spawn troupTransport;
		};
		last_battlegroup_size = last_battlegroup_size + 1;
	} foreach _selected_opfor_battlegroup;

	if ( liberation_csat_aggressivity > 0.9 ) then {
		[([markerpos _spawn_marker] call grad_liberation_common_fnc_getNearestBluforObjective) select 0] spawn spawnAir;
	};
