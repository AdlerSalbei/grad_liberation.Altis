...
while {dialog && (alive player)} do {
	...
	if (saveSectorSetting == 1) then {
		...
		waitUntil {sleep 0.5; (!(_selectedSector isEqualTo (liberation_production select _listselect)))};
	};
	...
	_selectedSector = +(liberation_production select _listselect);
	...
	if ((count (_selectedSector select 3)) > 0) then {
		_storage = ((nearestObjects [((_selectedSector select 3) select 0), [liberation_small_storage_building], 25]) select 0);
		_crateCount = count (attachedObjects _storage);
		_crateMax = count (liberation_small_storage_positions);
