...
if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] iedManager.sqf for %1 spawned on: %2", markerText _sector, debug_source];_text remoteExec ["diag_log",2];};
...
if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] iedManager.sqf -> spawning IED %1 at %2", _number, markerText _sector];_text remoteExec ["diag_log",2];};

if (_number > 0) then {
	[_sector, _radius, _number - 1] spawn iedManager;
};
...
if (!(isnull _roadobj)) then {
	...
	if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] iedManager.sqf -> IED %1 spawned at %2", _number, markerText _sector];_text remoteExec ["diag_log",2];};
	...
	if (liberation_asymmetric_debug > 0) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] iedManager.sqf -> _roadobj is Null for IED %1 at %2", _number, markerText _sector];_text remoteExec ["diag_log",2];};
};

if ((liberation_asymmetric_debug > 0) && !(isNull _roadobj)) then {private _text = format ["[KP LIBERATION] [ASYMMETRIC] iedManager.sqf -> exited IED %1 loop at %2", _number, markerText _sector];_text remoteExec ["diag_log",2];};
