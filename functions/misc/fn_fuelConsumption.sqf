/*
[] call grad_liberation_common_fnc_fuelConsumption.sqf
Author: Wyqer
...
player addEventHandler ["GetInMan", {[ _this select 2] execVM "scripts\[] call grad_liberation_common_fnc_fuelConsumption.sqf";}];
*/

//CONFIG
// Time in Minutes till a full tank depletes when the vehicle is standing with running engine
private _kp_neutral_consumption = liberation_fuel_neutral;
// Time in Minutes till a full tank depletes when the vehicle is driving
private _kp_normal_consumption = liberation_fuel_normal;
// Time in Minutes till a full tank depletes when the vehicle is driving at max speed
private _kp_max_consumption = liberation_fuel_max;

/*
DO NOT EDIT BELOW
*/

if (isNil "[] call grad_liberation_common_fnc_fuelConsumption_vehicles") then {
	[] call grad_liberation_common_fnc_fuelConsumption_vehicles = [];
};

if (!((_this select 0) in [] call grad_liberation_common_fnc_fuelConsumption_vehicles)) then {
	[] call grad_liberation_common_fnc_fuelConsumption_vehicles pushBack (_this select 0);
	...
	[] call grad_liberation_common_fnc_fuelConsumption_vehicles deleteAt ([] call grad_liberation_common_fnc_fuelConsumption_vehicles find (_this select 0));
};